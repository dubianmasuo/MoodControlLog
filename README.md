...
## 🚧 迭代记录
* **2025年12月22日**：修复导入数据时无有效记录的bug、修复自动保存逻辑与有效记录判断逻辑的冲突问题、移除切换日期时强制滚动到顶部、解决导入数据时同步更新字符计数器
* **2026年1月6日**：使用了 getTimezoneOffset() 手动计算时区导致日期提前一天，改为直接本地时间的值；检查并修复 CSV 导入时的日期处理；添加了 parseCSVLine 函数，正确处理字段内的逗号、引号和换行符
